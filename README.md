...
The agent exposes four literature tools to the model: `search_papers` (semantic retrieval over the Qdrant corpus), `fetch_arxiv` (pull fresh metadata from arXiv and index it when the corpus is thin), `get_paper_details` (abstract-level evidence for specific papers), and `get_full_text` (download a paper PDF and read its body: methods and results, not just the abstract). When `EXA_API_KEY` is configured, recency-sensitive runs must attempt `web_search` before literature tools are unlocked. The server anchors that query to the effective evidence date and original question, caps it at five highlighted allow-listed results, and rejects current-model synthesis with no grounded `web-N` citation. Expected web failure unlocks the arXiv fallback with an explicit caveat. The model otherwise composes the evidence and writes the cited decision brief as its final turn. Full-text fetching runs in-process (pypdf), so it stays deployable with no external paper service.
