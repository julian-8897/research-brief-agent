...
- `/papers/search` (via `ResearchTools.semantic_search`) adds two coverage behaviors, both toggleable per-request (`expand`/`backfill` query params) or by settings: query expansion (`src/agent/query_expansion.py`, HyDE-style — short keyword queries are rewritten into an abstract-like sentence before embedding, since SPECTER2 ranks descriptive text better) and arXiv backfill (fetch+index fresh papers for the query so the endpoint has coverage instead of only ranking the existing corpus). Backfill re-embeds fetched papers each call, so it adds real latency; it is the raw endpoint's analogue of the agent's discover→search loop. Residual ranking noise (token-match false positives) is a SPECTER2 precision limit that a cross-encoder reranker would address next.
