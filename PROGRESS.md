...
The core product works end-to-end and is **runnable locally today**. It is **not yet deployed** to the stated Fly.io target, and the real agent loop needs an LLM API key (without one it returns a deterministic fallback memo). Verified this session: app boots, `/health` 200, `/ingest` returns JSON errors for arXiv failures, the static workflow UI streams `/briefs/stream`, and a full interactive DeepSeek V4 run produced a cited memo after reading full text. Normal live synthesis is now gated on successful full-text evidence when retrieved papers exist; if the run cannot meet that contract before budget/failure, the stream emits explicit degraded/error events instead of silently looking successful.
...
- **Regression coverage:** full suite is now 36 passing with one Starlette/httpx deprecation warning, ruff clean. New tests cover the full-text gate, provider failure events, full-text success/error counters, arXiv id normalization, and full-text budget enforcement.

## Reliability review: remaining brittle points

These are the highest-priority guard-rail gaps still open.

1. **Live token cost is still high.** DeepSeek V4 now reaches `get_full_text` and writes a real memo, but the final validation still used ~70k input tokens. Transcript trimming and result compaction should be the next cost-control pass.
...
- Tests: 36 passing with one Starlette/httpx deprecation warning, ruff clean. Includes faked-SDK tests for both providers' wire translation and `tool_choice`, plus full-text evidence-gate and degraded-fallback regressions.
...
- DeepSeek V4 live behaviour is improved and revalidated, but token use remains high enough to justify transcript trimming/result compaction.
...
1. Trim/compact the live transcript: keep recent full payloads, summarize older search/tool results, and avoid re-sending large full-text bodies after the evidence requirement is met.
...
6. Build and smoke-test the Docker Compose stack against live Qdrant.
7. Add `fly.toml` + a documented deploy path; bake or cache the SPECTER model to fix cold start.
